...
export class Migration20260501150000 extends Migration {
  override up(): void | Promise<void> {
    this.addSql(
      `create table "shared_mailbox_group_item" ("handle" varchar(64) not null, "title" varchar(128) not null, "description" varchar(256) null, "icon" varchar(64) not null default 'mdi-email-lock-outline', "color" varchar(32) not null default '#1565C0', "is_active" boolean not null default true, "created_at" timestamptz not null, "updated_at" timestamptz not null, primary key ("handle"));`,
    );
    this.addSql(
      `create table "shared_mailbox_item" ("handle" serial primary key, "title" varchar(128) not null, "email" varchar(256) not null, "description" varchar(256) null, "provider" varchar(32) not null default 'azure', "is_active" boolean not null default true, "group_handle" varchar(64) null, "created_at" timestamptz not null, "updated_at" timestamptz not null);`,
    );
    this.addSql(
      `create table "shared_mailbox_group_item_persons" ("shared_mailbox_group_item_handle" varchar(64) not null, "person_item_handle" int not null, primary key ("shared_mailbox_group_item_handle", "person_item_handle"));`,
    );
