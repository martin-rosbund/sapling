...
export class Migration20251012145709 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table \`right_item\` (\`entity_handle\` text not null, \`can_read\` integer not null default true, \`can_insert\` integer not null default true, \`can_update\` integer not null default true, \`can_delete\` integer not null default true, \`can_show\` integer not null default true, \`created_at\` datetime null, \`updated_at\` datetime null, constraint \`right_item_entity_handle_foreign\` foreign key(\`entity_handle\`) references \`entity_item\`(\`handle\`) on delete cascade on update cascade, primary key (\`entity_handle\`));`);
