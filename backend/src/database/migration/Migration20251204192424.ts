...
export class Migration20251204192424 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table \`webhook_authentication_api_key_item\` (\`handle\` integer not null primary key autoincrement, \`description\` text not null, \`header_name\` text not null, \`api_key\` text null, \`created_at\` datetime not null, \`updated_at\` datetime not null, unique (\`handle\`));`);

    this.addSql(`create table \`webhook_authentication_oauth2item\` (\`handle\` integer not null primary key autoincrement, \`description\` text not null, \`client_id\` text not null, \`client_secret\` text null, \`token_url\` text null, \`scope\` text null, \`created_at\` datetime not null, \`updated_at\` datetime not null, unique (\`handle\`));`);
