...
export class Migration20251031095242 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table \`ticket_time_tracking_item\` (\`id\` integer not null primary key autoincrement, \`person_handle\` text not null, \`ticket_handle\` text not null, \`von\` datetime not null, \`bis\` datetime not null, \`created_at\` datetime not null, \`updated_at\` datetime not null, constraint \`ticket_time_tracking_item_person_handle_foreign\` foreign key(\`person_handle\`) references \`person_item\`(\`handle\`) on update cascade, constraint \`ticket_time_tracking_item_ticket_handle_foreign\` foreign key(\`ticket_handle\`) references \`ticket_item\`(\`handle\`) on update cascade);`);
