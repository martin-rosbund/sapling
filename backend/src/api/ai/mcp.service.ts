...
export class McpService {
  ...
  async listActiveTools(): Promise<McpToolDescriptor[]> {
  ...
  async tryExecuteInlineToolCommand(
    ...
    const targetConfig = parsedCommand.serverName
      ? configs.find(
          (config) =>
            config.name.trim().toLowerCase() ===
            parsedCommand.serverName?.trim().toLowerCase(),
        )
      : undefined;
    ...
    for (const config of candidateConfigs) {
      try {
        const result = await this.callTool(config, parsedCommand.toolName, parsedCommand.arguments);
        return {
