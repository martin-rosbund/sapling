...
import type { EntityItem, PermissionItem, PersonItem, RoleItem, RoleStageItem } from '../../entity/entity';
...
const PERMISSION_FIELDS: PermissionType[] = ['allowShow', 'allowRead', 'allowInsert', 'allowUpdate', 'allowDelete'];
...
export function useSaplingPermission() {
  ...
  const permissionIsLoading = computed(() => genericStore.getState('permission').isLoading || isBootstrapping.value);
  ...
  const availablePersonsForSelectedRole = computed<PersonItem[]>(() => {
    ...
    return persons.value
      .filter((person) => !(person.roles || []).some((role) => String(typeof role === 'object' ? role.handle : role) === roleHandle))
      .map((person) => ({
        ...person,
  ...
  const dashboardStats = computed(() => ({
    ...
    enabledPermissionCount: roles.value.reduce((total, role) => total + getEnabledPermissionCount(role), 0),
  }));
  ...
  watch(filteredRoles, (nextRoles) => {
    ...
    if (selectedRoleHandle.value == null || !nextRoles.some((role) => role.handle === selectedRoleHandle.value)) {
  ...
  watch(hasUnsavedPermissionChanges, (hasChanges) => {
    ...
    if (hasChanges && (permissionSaveState.value === 'saved' || permissionSaveState.value === 'error')) {
  ...
  function setPermission(role: RoleItem, item: EntityItem, type: PermissionType, value: boolean) {
    ...
    if (!baselinePermission && !isPermissionRecordEnabled(permission)) {
      role.permissions = (role.permissions || []).filter((entry) => getPermissionEntityHandle(entry) !== item.handle);
    }
  ...
  async function savePermissionChanges() {
    ...
    try {
      ...
      permissionSaveError.value = error instanceof Error ? error.message : 'The permission changes could not be saved.';
      throw error;
    }
  }
  ...
  async function confirmRemovePersonFromRole() {
    if (!deleteDialog.person || !deleteDialog.role || deleteDialog.person.handle == null || deleteDialog.role.handle == null) {
  ...
  function getEnabledPermissionCount(role: RoleItem): number {
    return (role.permissions || []).reduce((total, permission) => total + PERMISSION_FIELDS.filter((field) => permission[field] === true).length, 0);
  }
  ...
  function applyMembershipChange(person: PersonItem, role: RoleItem, shouldAdd: boolean) {
    roles.value = roles.value.map((entry) => updateRoleMembership(entry, role.handle, person, shouldAdd));
    originalRoles.value = originalRoles.value.map((entry) => updateRoleMembership(entry, role.handle, person, shouldAdd));
    ...
    persons.value = persons.value.map((entry) => {
      ...
      if (shouldAdd) {
        if (!nextRoles.some((roleEntry) => String(typeof roleEntry === 'object' ? roleEntry.handle : roleEntry) === String(role.handle))) {
      ...
      return {
        ...entry,
        roles: nextRoles.filter((roleEntry) => String(typeof roleEntry === 'object' ? roleEntry.handle : roleEntry) !== String(role.handle)),
      };
    });
  }
  ...
  async function persistRolePermissions(role: RoleItem) {
    ...
    for (const entity of entities.value) {
      ...
      try {
        ...
        const baselinePermission = originalRole ? getPermissionRecord(originalRole, entity.handle) : undefined;
        ...
        const permissionHandle = getPermissionRecordHandle(currentPermission) ?? getPermissionRecordHandle(baselinePermission);
        if (permissionHandle == null) {
          continue;
        }

        const updatedPermission = await ApiGenericService.update<PermissionItem>('permission', permissionHandle, permissionState);
  ...
  function assignPermissionResponse(role: RoleItem, entityHandle: string, permission: PermissionItem) {
  ...
  function getBaselinePermission(roleHandle: number | null, entityHandle: string): PermissionItem | undefined {
  ...
  function getPermissionRecord(role: RoleItem, entityHandle: string): PermissionItem | undefined {
    return role.permissions?.find((permission) => getPermissionEntityHandle(permission) === entityHandle);
  }

  function getPermissionEntityHandle(permission: PermissionItem): string {
    return typeof permission.entity === 'object' ? permission.entity.handle : String(permission.entity);
  }
  ...
  function getPermissionRecordHandle(permission?: PermissionItem): number | string | null {
    ...
    return 'handle' in permission ? (permission.handle as number | string | null | undefined) ?? null : null;
  }
  ...
  function updateRoleMembership(role: RoleItem, targetRoleHandle: number | null, person: PersonItem, shouldAdd: boolean): RoleItem {
