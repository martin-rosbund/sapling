...
export function useSaplingKpiSparkline(kpi: MaybeRefOrGetter<KPIItem | null | undefined>) {
  ...
  const value = computed(() =>
    data.value.map((dataPoint) => normalizeKpiNumericValue(dataPoint.value)),
  )
  const hasData = computed(() => value.value.length > 0)
  const firstValue = computed(() => (value.value.length > 0 ? value.value[0] : null))
  const lastValue = computed(() =>
    value.value.length > 0 ? value.value[value.value.length - 1] : null,
  )
  ...
  const averageValue = computed(() => {
    ...
    return Math.round(
      value.value.reduce((sum, currentValue) => sum + currentValue, 0) / value.value.length,
    )
  })
