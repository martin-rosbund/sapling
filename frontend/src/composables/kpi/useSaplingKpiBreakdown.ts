...
export function useSaplingKpiBreakdown(kpi: MaybeRefOrGetter<KPIItem | null | undefined>) {
  const { t } = useI18n()
  ...
  const leadShareLabel = computed(() => {
    ...
    return t('kpi.shareLabel', {
      value: Math.round((leadItem.value.value / totalValue.value) * 100),
    })
  })
  const categoryCountLabel = computed(() =>
    t('kpi.segmentCountLabel', { count: items.value.length }),
  )
