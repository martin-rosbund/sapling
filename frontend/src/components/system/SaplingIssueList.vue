	...
	<v-col cols="12" md="6">
		<section class="sapling-issue-stream" :class="`sapling-issue-stream--${status}`">
			...
			<div v-if="isLoading" class="sapling-issue-stream__loading">
				<v-skeleton-loader class="sapling-issue-stream__skeleton glass-panel" type="article, actions" />
				<v-skeleton-loader class="sapling-issue-stream__skeleton glass-panel" type="article, actions" />
			</div>
...
const emptyStateKey = computed(() => (props.status === 'open' ? 'issue.noOpenIssues' : 'issue.noClosedIssues'));
const streamIcon = computed(() => (props.status === 'open' ? 'mdi-progress-wrench' : 'mdi-check-all'));
