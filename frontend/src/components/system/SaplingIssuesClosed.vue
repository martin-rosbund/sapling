<template>
	<v-col cols="12" md="6">
		<h3 class="mb-4">{{ $t('issue.closedIssues') }}</h3>
		...
		<div v-else>
			<v-card v-for="issue in closedIssues" :key="'closed-' + issue.id" class="mb-6 sapling-bug-card glass-panel" elevation="3">
				<v-card-title class="d-flex align-center justify-space-between">
					...
					<v-chip color="grey" size="small">{{ $t('issue.closed') }}</v-chip>
				</v-card-title>
...
import { useSaplingClosedIssues } from '@/composables/system/useSaplingIssuesClosed';
import VMarkdown from 'vue-markdown-render';
const { closedIssues, isLoading } = useSaplingClosedIssues();
</script>
